...
static void cdcacm_data_tx_cb(usbd_device *usbd_dev, uint8_t ep){
  ...
  buf[0]='a';
  buf[1]='b';
  usbd_ep_write_packet(usbd_dev,0x82,buf,2);
}
...
#define SYSTICK_RELOAD_PERIOD 8999
...
void sys_tick_handler(void){
  ...
  if(startup_counter>SYSTICK_STARTUP_DELAY){
    if((out_buf_len>0)&&(NULL!=usbd_dev))
      usbd_ep_write_packet(usbd_dev,0x82,out_buf,out_buf_len);
  }else{
    startup_counter++;
  }
}
